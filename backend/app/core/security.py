...
OAuth issues the same JWTs, so the rest of the app never has to care how a
user signed in — it just reads current_user from the token."""
import base64
import hashlib
import hmac
import json
...
def create_token(user_id: int, extra: dict | None = None) -> str:
    ...
    payload = {"sub": str(user_id), "iat": now,
               "exp": now + settings.auth.jwt_ttl_days * 86400, **(extra or {})}
    segments = [_b64url(json.dumps(header, separators=(",", ":")).encode()),
                _b64url(json.dumps(payload, separators=(",", ":")).encode())]
    signing_input = ".".join(segments).encode()
    sig = hmac.new(settings.auth.jwt_secret.encode(), signing_input, hashlib.sha256).digest()
...
def decode_token(token: str) -> dict | None:
    ...
    try:
        ...
        expected = hmac.new(settings.auth.jwt_secret.encode(), signing_input, hashlib.sha256).digest()
